...
{
    struct Env *current_process = get_cpu_proc(); // Get the current process

    // Set the current process status to BLOCKED
    current_process->env_status = ENV_BLOCKED;

    // Enqueue the current process to the semaphore's queue
    enqueue(queue, current_process);

    // Release the lock (spinlock)
      *lock = 0;

    // Acquire the process queue lock before scheduling
    acquire_spinlock(&ProcessQueues.qlock);


    // Schedule the next process (context switch)
    sched();

    // Release the process queue lock after scheduling
    release_spinlock(&ProcessQueues.qlock);

    // Note: Optionally, acquire the semaphore lock again here if needed
    //acquire_spinlock(lock);
}


void sys_signal_semaphore(struct Env_Queue *queue, uint32 *lock)
{
    	acquire_spinlock(&ProcessQueues.qlock);
    	struct Env* first_process;
    	first_process=dequeue(queue);
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_get_id:
		return sys_get_id_by_virtual_address((void*)a1);
		break;
	...
	case SYS_wait_semaphore:
		sys_wait_semaphore((struct Env_Queue*) a1,(uint32 *)a2);
				return 0;
				break;

	case SYS_signal_semaphore:
			sys_signal_semaphore((struct Env_Queue*) a1,(uint32 *)a2);
					return 0;
					break;
	}
