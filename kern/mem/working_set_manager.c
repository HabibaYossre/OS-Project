...
inline void Shrouk_env_page_ws_invalidate_fast(struct Env* e, uint32 virtual_address) {
    if (!isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)) {
        // Check page permissions for the given virtual address
        if (pt_get_page_permissions(e->env_page_directory, virtual_address) & PERM_PRESENT) {
            ...
            if (Habiba != NULL) { // Validate Habiba
                if (e->page_last_WS_element == Habiba) {
                    e->page_last_WS_element = LIST_NEXT(Habiba); // Update page_last_WS_element if necessary
                }
                LIST_REMOVE(&(e->page_WS_list), Habiba); // Remove from working set list
                unmap_frame(e->env_page_directory, Habiba->virtual_address); // Unmap the frame
                kfree(Habiba); // Free the working set element
            }
        }
    } else { // Handling for LRU case
        // Check if the page is present
        if (pt_get_page_permissions(e->env_page_directory, virtual_address) & PERM_PRESENT) {
            uint32* ptr_page_table = NULL;
            struct WorkingSetElement* wse = get_frame_info(e->env_page_directory, virtual_address, &ptr_page_table)->Eman;

            if (wse != NULL) { // Validate wse
                LIST_REMOVE(&(e->ActiveList), wse); // Remove from active list
                unmap_frame(e->env_page_directory, wse->virtual_address); // Unmap the frame
                kfree(wse); // Free the working set element

                // Check for a candidate in the SecondList
                struct WorkingSetElement* ptr_tmp_WS_element = LIST_FIRST(&(e->SecondList));
                if (ptr_tmp_WS_element != NULL) {
                    LIST_REMOVE(&(e->SecondList), ptr_tmp_WS_element); // Remove from second list
                    LIST_INSERT_TAIL(&(e->ActiveList), ptr_tmp_WS_element); // Add to active list
                    pt_set_page_permissions(e->env_page_directory, ptr_tmp_WS_element->virtual_address, PERM_PRESENT, 0); // Set permissions
                }
            }
        }
    }
